...
description: Set up sign-in and sign-out flows in a Node.js application using the ThunderID Node.js SDK
---
...
## 5. Verify the Sign-Out Result

After the identity provider redirects back to your `afterSignOutUrl`, check whether the sign-out succeeded.

```ts title="src/routes/home.ts" showLineNumbers
import { LegacyThunderIDNodeClient } from '@thunderid/node'

app.get('/', (req, res) => {
  const currentUrl = `${req.protocol}://${req.get('host')}${req.originalUrl}`

  if (LegacyThunderIDNodeClient.isSignOutSuccessful(currentUrl)) {
    // Sign-out completed successfully
  }

  if (LegacyThunderIDNodeClient.didSignOutFail(currentUrl)) {
